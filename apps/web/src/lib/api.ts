...
export const launch = {
  ...
  validateTargeting: (productId: number, templateId: number, structure: LaunchStructure | null) => request<{ requests: number; checks: { adSet: string; ok: boolean; message: string | null; code: number | null; subcode: number | null; estimate: { users_lower?: number; users_upper?: number } | null }[] }>(`/products/${productId}/launch-validate`, { method: 'POST', body: JSON.stringify({ templateId, structure, acknowledge: [] }) }),
